...
$(document).on("dblclick", ".item-slot", function(e){
    ...
    if(ItemData) {
        Inventory.Close();
        $.post("https://eko-inventory/UseItem", JSON.stringify({
...
$(document).on('click', '.weapon-attachments-back', function(e){
    e.preventDefault();
    $("#eko-inventory").css({"display":"block"});
    $("#eko-inventory").animate({
        left: 0+"vw"
    }, 200);
...
function FormatAttachmentInfo(data) {
    $.post("https://eko-inventory/GetWeaponData", JSON.stringify({
        weapon: data.name,
...
function handleAttachmentDrag() {
    ...
    $(".weapon-attachments-remove").droppable({
        ...
        drop: function(event, ui) {
            $.post('https://eko-inventory/RemoveAttachment', JSON.stringify({
...
$(document).on('click', '#weapon-attachments', function(e){
    ...
    if (!Inventory.IsWeaponBlocked(ClickedItemData.name)) {
        ...
        $("#eko-inventory").animate({
            ...
            $("#eko-inventory").css({"display":"none"})
        });
        $(".weapon-attachments-container").animate({
            left: 0+"vw"
        }, 200);
        ...
        $.post('https://eko-inventory/Notify', JSON.stringify({
...
function handleDragDrop() {
    ...
    $("#item-give").droppable({
        ...
        drop: function(event, ui) {
            ...
            if(fromData.amount > 0) {
                $.post("https://eko-inventory/GiveItem", JSON.stringify({
                    inventory: fromInventory,
                    item: fromData,
    ...
    $(document).on('click', '#item-close', function(e){
        e.preventDefault();
        $.post("https://eko-inventory/CloseInventory", JSON.stringify({}));
        Inventory.Close();
    });
    ...
    $("#item-drop").droppable({
        ...
        drop: function(event, ui) {
            ...
            $.post("https://eko-inventory/DropItem", JSON.stringify({
                inventory: fromInventory,
                item: fromData,
                amount: parseInt(amount),
            }));
...
$(document).on('click', '.CombineItem', function(e){
    ...
    if (combineslotData.toData.combinable.anim != null) {
        $.post('https://eko-inventory/combineWithAnim', JSON.stringify({
        ...
        $.post('https://eko-inventory/combineItem', JSON.stringify({
...
function optionSwitch($fromSlot, $toSlot, $fromInv, $toInv, $toAmount, toData, fromData) {
    ...
    $.post("https://eko-inventory/SetInventoryData", JSON.stringify({
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    if (fromData !== undefined && fromData.amount >= $toAmount) {       
        ...
        if((toData != undefined || toData != null) && toData.name == fromData.name && !fromData.unique) {
            ...
            $.post("https://eko-inventory/PlayDropSound", JSON.stringify({}));
            $.post("https://eko-inventory/SetInventoryData", JSON.stringify({
                fromInventory: $fromInv.attr("data-inventory"),
                toInventory: $toInv.attr("data-inventory"),
                fromSlot: $fromSlot,
            ...
            if (fromData.amount == $toAmount) {
                if (toData != undefined && toData.combinable != null && isItemAllowed(fromData.name, toData.combinable.accept)) {
                    $.post('https://eko-inventory/getCombineItem', JSON.stringify({item: toData.combinable.reward}), function(item){
...
function InventoryError($elinv, $elslot) {
    ...
    $.post("https://eko-inventory/PlayDropFail", JSON.stringify({}));
}
...
(() => {
    ...
    Inventory.Clickables = function(){
        ...
        $('.cloth').find('img').click(function(){
            $.post("https://eko-inventory/ChangeComponent", JSON.stringify({
    ...
    Inventory.Necessary = function(){
        ...
        $('.question').attr('data-html','true').attr('title','This is an complex inventory system made for QBCore. <li>Drag an item to use button to use the item</li>'+
...
$(document).on('click', '#rob-money', function(e){
    ...
    $.post('https://eko-inventory/RobMoney', JSON.stringify({
        TargetId: TargetId
    }));
    $("#rob-money").remove();
});